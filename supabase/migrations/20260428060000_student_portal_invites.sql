...
-- ----------------------------------------------------------------------------
-- Adds the gym-admin-driven onboarding flow for student portal access:
--   * students.portal_access_enabled flag
--   * student_portal_invites table + RLS
--   * RPCs: lookup / create / accept / cancel / resend / disable
--   * Updated students_select_linked_portal policy that gates portal reads
--     on portal_access_enabled = true
--
-- The migration is idempotent and safe on partial hosted DBs. It does NOT
-- recreate is_org_member / is_gym_member / has_org_role - those helpers are
-- assumed to exist from the initial schema (or repair_hosted_minimum.sql).
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. students.portal_access_enabled
-- ----------------------------------------------------------------------------
alter table public.students
  add column if not exists portal_access_enabled boolean not null default false;

-- ----------------------------------------------------------------------------
-- 2. student_portal_invites
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. RLS policies for student_portal_invites
-- ----------------------------------------------------------------------------
-- Staff (org owners, gym members for the invite's gym, platform admin) can
-- read and manage. Anonymous lookups go through the lookup RPC (security
...
-- ----------------------------------------------------------------------------
-- 4. Tighten students RLS: gym-scoped staff + portal_access_enabled gate
-- ----------------------------------------------------------------------------
-- The initial migration created generic org-wide policies via a loop. Replace
-- the SELECT side with a gym-scoped + portal-aware policy so users can only
-- see students for the gyms they actually staff (or that they are linked to
-- via student/parent links with portal_access_enabled = true).
...
-- Gym-scoped insert/update/delete. Drop the wider org-only versions and add
-- policies that require gym staff membership or org-owner / platform-admin.
...
-- ----------------------------------------------------------------------------
-- 5. can_manage_gym_students helper
-- ----------------------------------------------------------------------------
-- True when caller is platform admin, org owner of the gym's org, or any
-- active gym_members row for the gym.
...
-- ----------------------------------------------------------------------------
-- 6. lookup_student_portal_invite_by_token
-- ----------------------------------------------------------------------------
-- Public endpoint used by /portal/accept-invite/:token landing page. Returns
-- only the metadata needed to render the page; never the raw invite row.
...
-- ----------------------------------------------------------------------------
-- 7. create_student_portal_invite
-- ----------------------------------------------------------------------------
-- Gym staff create an invite for a student in their gym. Cancels any existing
-- pending invites for the student so the partial unique index is respected.
...
-- ----------------------------------------------------------------------------
-- 8. accept_student_portal_invite
-- ----------------------------------------------------------------------------
-- Called by the authenticated invited user. Validates the invite, links the
-- auth user to the student via student_user_links, flips
-- portal_access_enabled to true, and marks the invite accepted.
...
-- ----------------------------------------------------------------------------
-- 9. cancel_student_portal_invite
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10. resend_student_portal_invite
-- ----------------------------------------------------------------------------
-- Cancels the current pending invite (if any) and creates a fresh one. Email
-- defaults to the existing invite's email so resending without arguments is
-- safe from the UI.
...
begin
  ...
  return query
  select * from public.create_student_portal_invite(v_invite.student_id, v_invite.email);
end;
$$;
...
-- ----------------------------------------------------------------------------
-- 11. disable_student_portal_access
-- ----------------------------------------------------------------------------
-- Revokes all portal access for a student: clears student_user_links rows,
-- cancels any pending invites, and flips portal_access_enabled to false.
